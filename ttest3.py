...
def medaf(portfolio_sector : dict, beta_results : dict,
          chosen_sectors_dict : dict, rf= 0.04) -> dict:
    try :
            ...
            for sector in portfolio_sector.keys() :
                if key == sector : 
                    ...
                    for i in range(len(chosen_sectors_dict[key])):
                        ...
                        print(f"market_risk_prime {market_risk_prime}")
...
def calculate_beta(market_closing_df: DataFrame, stocks_closing_df: DataFrame, 
    ...
    print(f"\nmarket_closing_df_filtered\n {market_closing_df_filtered}")
    print(f"\stocks_closing_df_filtered\n {stocks_closing_df_filtered}")
...
def annulized_return(totals_Cu_Rre:dict,
    ...
    for name in totals_Cu_Rre.keys():
       ...
       if totals_Cu_Rre[name] <= -100:  # Check for more than 100% loss
            five_year_annualized[name_apt] = None  # Or handle accordingly
       else:
